...
% Initialisation of POI Libs for iOS 
...
try
    if Parameter ~= 4 && Parameter ~= 5 && Parameter ~= 10 && Parameter ~= 11 && Parameter ~= 12
        xlwrite([pathname filename],{Title},Sheet,'A1:A1')
        xlwrite([pathname filename],GM.Names,Sheet,'B1')
        waitbar(1/4,h,'Exporting Data...')
        xlwrite([pathname filename],DATAmax,Sheet,'B2')
        xlwrite([pathname filename],(1:Nst)',Sheet,'A2')
        waitbar(1/2,h,'Exporting Data...')
        xlwrite([pathname filename],{Title},Sheet,['A' num2str(4+Nst) ':A' num2str(4+Nst)])
        xlwrite([pathname filename],GM.Names,Sheet,['B' num2str(4+Nst)])
        waitbar(3/4,h,'Exporting Data...')
        xlwrite([pathname filename],DATAmin,Sheet,['B' num2str(5+Nst)])
        xlwrite([pathname filename],(1:Nst)',Sheet,['A' num2str(5+Nst)])
        waitbar(1,h,'Exporting Data...')
    elseif Parameter == 10 || Parameter == 11
        xlwrite([pathname filename],HEADER,Sheet,'A1')
        waitbar(1/4,h,'Exporting Data...')
        xlwrite([pathname filename],NPTS,Sheet,'A2')
        waitbar(2/4,h,'Exporting Data...')
        xlwrite([pathname filename],HEADER2,Sheet,'A3')
        waitbar(3/4,h,'Exporting Data...')
        xlwrite([pathname filename],DATA,Sheet,'A4')
        waitbar(1,h,'Exporting Data...')
    elseif Parameter == 12
        xlwrite([pathname filename],{Title},Sheet,'A1:A1')
        waitbar(1/3,h,'Exporting Data...')
        xlwrite([pathname filename],GM.Names,Sheet,'A2')
        waitbar(2/3,h,'Exporting Data...')
        xlwrite([pathname filename],DATA,Sheet,'A3')
        waitbar(1,h,'Exporting Data...')
    else
        xlwrite([pathname filename],{Title},Sheet,'A1:A1')
        waitbar(1/3,h,'Exporting Data...')
        xlwrite([pathname filename],GM.Names,Sheet,'B1')
        waitbar(2/3,h,'Exporting Data...')
        xlwrite([pathname filename],DATA,Sheet,'B2')
        xlwrite([pathname filename],(1:Nst)',Sheet,'A2')
        waitbar(1,h,'Exporting Data...')
    end
    close(h)
